# QuestWordHunter — German Voiceover: Battle for Azeroth

The German a quest giver says out loud, for Battle for Azeroth: quest ids
**48,159–56,054**.

**20,849 clips, 23.3 hours.** Both are counted from this pack's own duration table
in `Part.lua` — the table the engine plays from — so they are what the pack can
...
`sounds/` is committed, and `.gitignore` says why. It is 414 MB here, out of
...
```
python Tools/build_pack.py --only Azeroth --out "…/Interface/AddOns"
```
